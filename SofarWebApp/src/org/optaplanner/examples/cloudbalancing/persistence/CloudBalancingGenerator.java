...
//        generator.writeCloudBalance(20, 60);
    ...
    public CloudBalance createCloudBalance(String inputId, int computerListSize, int processListSize) {
        ...
        BigInteger possibleSolutionSize = BigInteger.valueOf(cloudBalance.getBusList().size()).pow(
                cloudBalance.getRouteList().size());
    ...
    private void createComputerList(CloudBalance cloudBalance, int computerListSize) {
        List<CloudBus> computerList = new ArrayList<>(computerListSize);
        for (int i = 0; i < computerListSize; i++) {
            CloudBus computer = generateComputerWithoutId();
    ...
    public CloudBus generateComputerWithoutId() {
        CloudBus computer = new CloudBus();
        int cpuPowerPricesIndex = random.nextInt(CPU_POWER_PRICES.length);
        computer.setCapacity(CPU_POWER_PRICES[cpuPowerPricesIndex].getHardwareValue());
        int memoryPricesIndex = distortIndex(cpuPowerPricesIndex, MEMORY_PRICES.length);
        computer.setTransfers(MEMORY_PRICES[memoryPricesIndex].getHardwareValue());
        int networkBandwidthPricesIndex = distortIndex(cpuPowerPricesIndex, NETWORK_BANDWIDTH_PRICES.length);
        computer.setTimeFinished(NETWORK_BANDWIDTH_PRICES[networkBandwidthPricesIndex].getHardwareValue());
    ...
    private void createProcessList(CloudBalance cloudBalance, int processListSize) {
        List<CloudRoute> processList = new ArrayList<>(processListSize);
        for (int i = 0; i < processListSize; i++) {
            CloudRoute process = generateProcessWithoutId();
            process.setId((long) i);
            processList.add(process);
        }
        cloudBalance.setRouteList(processList);
    }

    public CloudRoute generateProcessWithoutId() {
        CloudRoute process = new CloudRoute();
        int requiredCpuPower = generateRandom(MAXIMUM_REQUIRED_CPU_POWER);
        process.seDemand(requiredCpuPower);
        int requiredMemory = generateRandom(MAXIMUM_REQUIRED_MEMORY);
        process.setTravelTime(requiredMemory);
        int requiredNetworkBandwidth = generateRandom(MAXIMUM_REQUIRED_NETWORK_BANDWIDTH);
        process.setStartTime(requiredNetworkBandwidth);
    ...
    private void assureComputerCapacityTotalAtLeastProcessRequiredTotal(CloudBalance cloudBalance) {
        List<CloudBus> computerList = cloudBalance.getBusList();
        int cpuPowerTotal = 0;
        int memoryTotal = 0;
        int networkBandwidthTotal = 0;
        for (CloudBus computer : computerList) {
            cpuPowerTotal += computer.getCapacity();
            memoryTotal += computer.getTransfers();
            networkBandwidthTotal += computer.getTimeFinished();
        }
        ...
        for (CloudRoute process : cloudBalance.getRouteList()) {
            requiredCpuPowerTotal += process.getDemand();
            requiredMemoryTotal += process.getTravelTime();
            requiredNetworkBandwidthTotal += process.getStartTime();
        }
        int cpuPowerLacking = requiredCpuPowerTotal - cpuPowerTotal;
        while (cpuPowerLacking > 0) {
            CloudBus computer = computerList.get(random.nextInt(computerList.size()));
            int upgrade = determineUpgrade(cpuPowerLacking);
            computer.setCapacity(computer.getCapacity() + upgrade);
        ...
        while (memoryLacking > 0) {
            ...
            computer.setTransfers(computer.getTransfers() + upgrade);
        ...
        while (networkBandwidthLacking > 0) {
            ...
            computer.setTimeFinished(computer.getTimeFinished() + upgrade);
